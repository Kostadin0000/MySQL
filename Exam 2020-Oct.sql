...
CREATE TABLE products_stores (
    ...
    CONSTRAINT fk_mapped_products FOREIGN KEY (product_id)
        REFERENCES products (id),
    CONSTRAINT fk_mapped_stores FOREIGN KEY (store_id)
...
CREATE TABLE employees (
    ...
    manager_id INT,
    store_id INT NOT NULL,
    CONSTRAINT fk_employees_1 FOREIGN KEY (manager_id)
...
UPDATE employees AS e
    ...
    stores AS s ON s.id = e.store_id
        LEFT JOIN
    employees AS em ON e.manager_id = em.id 
SET 
    e.manager_id = 3,
...
-- 7 
SELECT 
    s.name, COUNT(ps.product_id) AS product_count, ROUND(AVG(p.price),2) AS `avg`
FROM
    stores AS s
        LEFT JOIN
